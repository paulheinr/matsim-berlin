...
//					use act end time if defined
				...
				if( durationFirst == null ){
					log.fatal( "Neither duration nor end time is defined for activity {} of agent {}. This should not happen, aborting!", first,
						p.getId() );
...
//				duration bins of first and last act have to match
				if( !splitFirst[1].equals( splitLast[1] ) ){
					log.fatal( "typical duration of first and last activity of person {} are not the same. This should not happen, aborting!",
						p.getId() );
					throw new IllegalStateException( "" );
				}
